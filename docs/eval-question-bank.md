...
| 4 | B. CRM 사실 정확도 | "코드 [실제 매물코드] 매물 정보 알려줘" (매물코드 TBD) | 실제 가격/위치/상태와 일치 |
...
| 15 | E. LM Studio 로컬 답변 (provider=lmstudio) | "복잡한 법률 질문" | 한계 인지 여부 관찰 (실패해도 정상 판정) |
...
- **4~6번 (CRM 사실 정확도)**: 실제 DB 매물 코드로 채워야 함. 다음 세션에 실제
  `rbs_homes_ops` CRM에서 매물 코드 1~2개를 조회해서 4번 문항에 반영 필요.
- **15번 (로컬 모델 법률 질문)**: "실패해도 정상 판정"으로 설계됨. 이 판정 기준이
  적절한지 다음 세션에 재확인 필요.
